  ...
  useEffect(() => {
    ...
    if (dateFilter === "today") {
      ...
      filtered = filtered.filter(
        (user) => new Date(user.enrolled_at) >= weekAgo
      );
    } else if (dateFilter === "month") {
      const monthAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
      filtered = filtered.filter(
        (user) => new Date(user.enrolled_at) >= monthAgo
      );
    }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-white">
              Futurs Traders en Finance de Marché
            </h1>
          </div>
          <p className="text-gray-400 text-lg">
            Candidats inscrits pour décrocher leur Summer chez Goldman Sachs, JP
            Morgan, BNP Paribas...
          </p>
        ...
        <div className="mb-6 space-y-4">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Rechercher par nom, email, école, téléphone..."
            ...
            <tbody className="divide-y divide-gray-800">
              {currentUsers.map((user, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {user.ecole ? (
                      <div className="text-sm text-gray-300">{user.ecole}</div>
                    ) : (
                      <div className="text-sm text-gray-500">-</div>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 max-w-xs">
                    {user.notes ? (
                      <div className="text-sm text-gray-300 truncate" title={user.notes}>
                        {user.notes}
                      </div>
                    ) : (
                      <div className="text-sm text-gray-500">-</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {user.newsletter ? (
                      <div className="text-sm text-green-500">✓ Inscrit</div>
                    ) : (
                      <div className="text-sm text-gray-500">-</div>
                    )}
                  </td>
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-between">
            <div className="text-sm text-gray-400">
              ...
              {filteredUsers.length} futurs traders
            </div>
